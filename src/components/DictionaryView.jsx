...
import { Book, Search, Copy, Check } from 'lucide-react';
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-8">
      ...
      <div className="bg-stone-100 dark:bg-stone-900 rounded-xl border border-stone-200 dark:border-stone-800 p-6 sm:p-8">
        <div className="max-w-3xl">
          <div className="flex items-center gap-2 mb-2">
            <Book size={18} className="text-cardinal-600 dark:text-cardinal-400" />
          ...
          <p className="text-stone-600 dark:text-stone-400 text-sm sm:text-base">
            From <span className="text-cardinal-600 dark:text-cardinal-400 font-semibold">TALK</span> to <span className="text-cardinal-600 dark:text-cardinal-400 font-semibold">FOAM</span>, decode the terms, places, traditions, and acronyms used daily across Knight Management Center.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredTerms.map((t, idx) => (
          <div
            key={t.term + idx}
            className="interactive-card bg-white dark:bg-stone-900 rounded-xl border border-stone-200 dark:border-stone-800 p-5 space-y-3 flex flex-col justify-between"
          >
            <div>
              <div className="flex items-center justify-between gap-2 mb-2">
                <span className="font-bold text-lg text-cardinal-600 dark:text-cardinal-400">
                  {t.term}
                </span>
                <button
                  onClick={() => copyTerm(t)}
                  className="p-2.5 -m-1 rounded-lg text-stone-400 hover:text-stone-600 hover:bg-stone-100 dark:hover:text-stone-200 dark:hover:bg-stone-800 active:scale-95 transition-all"
                  title="Copy term definition"
                  aria-label={`Copy definition for ${t.term}`}
                >
